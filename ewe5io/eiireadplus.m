function [FileContents, Other] = eiireadplus(filename)
%EIIREADPLUS Reads an Ecopath II text file and creates extra variables
%
%  [FileContents, Other] = eiiread(filename)
%
...
%
% If a second output is given, this function also creates the variables
% that are initialized when a .eii file is imported into EwE.
%
% Input variables:
%
...
% Output variables:
%
%   FileContents:   1 x 1 structure holding variables stored in the file.
...
%                                   (bh = b/area)
%
%   Other:          1 x 1 structure of other variables initialized when
%                   .eii file is read into EwE.  Most are arrays of
%                   zeros.  Don't know what all of them are used for yet.
...
[estimateWhat, ex, catches, b, bi, bh, bhi, ba, bai, babi, pb, ee, ge, ...
 ...
 immig, immigi, emigration, emigrationi, emigi, shadow, groupIsFish, ...
 groupIsInvert, iadult, ijuv, adJuvSelected, ainLw, binLw, loo, winf, ...
 vbK, t0, tcatch, tmax, adultGroup, assimEff, stanzaGroup, ttlx, cZero, ...
 cImmig, cEnv, cDecay, cInflow, cOutflow] = ...
                                                   deal(zeros(ngroup,1));
                                                      
areas = ones(ngroup,1);

[dc, dci, host, lhs, concTr] = deal(zeros(ngroup+1));
dc_c0 = zeros(ngroup+1,1);
dci_c0 = zeros(ngroup+1,1);

specie = cell(ngroup,1);

missing = zeros(ngroup,4);
h = zeros(ngroup+3,1);

[y, f, hlap, plap, niche, a, propLanded] = deal(zeros(ngroup));

q = zeros(ngroup+10,1);

df = zeros(ngroup,ndetritus);

lockedParameter = false(ngroup,1);

speciesCode = zeros(ngroup,2);

aul = zeros(ngroup+3);

% path(0:2*ngroup+2,1)

lastComp = zeros(2*ngroup+2,1);

if nlive > 0
    last = zeros(1*nlive);
end

poolColor = zeros(ngroup+2,1);

groupTaken = false(nlive);

growthInput = false(nlive,7);

catchCode = zeros(30,ngroup);

parLimit = zeros(1,5,ngroup);

cvPar = zeros(5,ngroup);
cvPar(1:4,:) = 0.1; % 0th row should also hold 0.1
cvPar(5,:) = 0.05;
...
grpsToShow = false(ngroup+ngear+2,1);
grpsToShow(1:ngroup+ngear) = true;

[det, flowToDet] = deal(zeros(ngroup+ngear));

dcDet = zeros(ngroup-nlive,ngear);

[cost, costPct] = deal(zeros(ngear,3));

gearName = cell(ngear+1);

[landing, discard, discardFate, propLanded, propDiscard, market] = ...
                                             deal(zeros(ngear,ngroup));

[epower, pcapBase, capDepreciate, capBaseGrowth, gearColor] = ...
                                                    deal(zeros(ngear,1));
...
FileContents = var2struct(ngroup, nlive, currUnit, currUnitIndex, ...
...
Other = var2struct(a, adJuvSelected, adultGroup, ainLw, assimEff, aul, ...
                   b, ba, babi, bh, binLw, bqb, cDecay, cEnv, cImmig, ...
                   cInflow, cOutflow, cZero, capBaseGrowth, ...
                   capDepreciate, catchCode, concTr, cost, cvPar, dc, ...
                   dcDet, dc_c0, det, detEaten, detPassedOn, ...
                   detPassedProp, e, ee, emigi, emigration, epower, ...
                   estimateWhat, f, flowToDet, ge, gearColor, ...
                   groupIsFish, groupIsInvert, groupTaken, growthInput, ...
                   grpsToShow, h, hlap, host, iadult, ijuv, immig, ...
                   inputToDet, last, lastComp, lhs, lockedParameter, ...
                   loo, m0, m2, mis, missing, ndetritus, ...
                   ngear, niche, noBqb, p, parLimit, pb, pcapBase, ...
                   plap, poolColor, propDiscard, propLanded, q, qb, ...
                   resp, speciesCode, stanzaGroup, sumDc, t0, tcatch, ...
                   timeUnitIndex, timeUnitName, tlSim, tmax, ttlx, vbK, ...
                   winf, y);
